...
type CartUIContextValue = {
  ...
  addOptimisticOrder: (order: Omit<OptimisticOrder, "tempId" | "createdAtMs">) => string;
...
export function CartUIProvider({ children }: { children: ReactNode }) {
  ...
  const addOptimisticOrder = useCallback((order: Omit<OptimisticOrder, "tempId" | "createdAtMs">) => {
    const tempId = `optimistic-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    setOptimisticOrders((prev) => [{ ...order, tempId, createdAtMs: Date.now() }, ...prev]);
  ...
  const value = useMemo(
    ...
    [open, setOpen, pulse, markInteracted, optimisticOrders, addOptimisticOrder, removeOptimisticOrder],
  );
